...
if numel(ser)>1
    ...
    if isfield(data.series,'traces')

        for z=1:numel(ser)
            tmp_time(z) = length(data.series(ser(z)).traces(1,1).values(1,:));
            tmp_trials(z) = length(data.series(ser(z)).traces(1,1).values(:,1)); % trials per series provided
        end
        ser_maxdatapoints= max(tmp_time); % index of series with the maximal time datapoints
        maxtrials= sum(tmp_trials); % sum of all the trials
        ...
        for z=1:numel(ser)
            for n=1:nCh;
                parse(z).ch(n).vm = data.series(ser(z)).traces(1,chNumbers(n)).values';
                parse(z).ch(n).mvm = mean(parse(z).ch(n).vm,2); % mean Vm trace
                parse(z).ch(n).units = data.series(ser(z)).traces(1,chNumbers(n)).unit;
                parse(z).ch(n).trialmeans = [means,mean(parse(z).ch(n).vm,1)];
            end
        end
        ...
        t.dt =data.series(ser(1)).traces(chNumbers(1)).timeVector; t.dt = t.dt(2);
        t.s = t.dt:t.dt:length(tmp_mat(:,1,1))*t.dt;
        t.ms = t.s.*1000;
    end
end